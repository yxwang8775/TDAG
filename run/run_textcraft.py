...
# from run_alfworld import convert_messages


output = open('./output/run_textcraft_dev3.txt', 'w')
sys.stdout = output
...
def plan_and_run(commands, task, idx, env, prompt, past_action_checkpoint=[], past_info_prop='', depth=1, num_runs=0,
                 verbose=True, comm_state=False, ttype=''):
    plan_list = []
    ...
    current_update_prompt = None

    if isinstance(task, str):  # execute. If fail, decompose
        if verbose: print('Starting... ' + task, ' at depth ' + str(depth))
        custom_ob = commands + '\nGoal: ' + task + '\n'

        print(f"custom_ob={custom_ob}")

        if len(info_prop):
            if verbose: print('Loading ...', info_prop)
            if LM in ["gpt-3.5-turbo", "gpt-4", "gpt-3.5-turbo-0301"]:
                r, succ, term, completion, act_history, num = textcraft_run(prompt, to_print=False,
                                                                            ob=custom_ob + '\n' + info_prop,
        ...
        result_prompt = f"""The following is a preliminary plan:\n"""
        current_update_prompt = plan_prompt + '\n' + custom_ob + result_prompt + plan
    ...
    for sub_task in plan_steps['steps']:
        ...
        r, succ, term, env, completion, act_history, _, decomp_plans, num, info_prop = plan_and_run(commands, sub_task,
                                                                                                    idx, env, prompt,
                                                                                                    ...
                                                                                                    ttype=ttype)  # Need to propogate info_prop here.
        ...
        if plan_steps['logic'].lower() == 'and' and not succ:
            ...
            if current_update_prompt is None:
                return r, succ, term, env, running_completion, action_checkpoint, depth, plan_list, num_runs, info_prop

            update_prompt = f"""But the plan failed when executing "{sub_task}". Please rewrite a plan and keep the format consistent with the initial plan."""
            update_prompt = current_update_prompt + update_prompt
            plan = plan_llm(update_prompt)
            ...
            if len(plan_steps['steps']) == 1:
                ...
                if 'logic' not in plan_steps.keys():
                    try:
                        ...
                        logic = "AND";
                        plan_steps['logic'] = logic
            r, succ, term, env, completion, act_history, _, decomp_plans, num, info_prop = plan_and_run(commands,
                                                                                                        plan_steps,
                                                                                                        idx, env,
                                                                                                        prompt,
                                                                                                        past_action_checkpoint=action_checkpoint,
...
dest_file = './results/textcraft/{}.json'.format(
    f'TDAG_maxd{max_depth}_hybrid_exec_runs{max_runs}_test_num{num_games}_dev3')
...
for idx in pbar:
    try:
        ...
        trace = []
        r, succ, term, env, trace, actions, depth, plans, num_runs, _ = plan_and_run(commands, task, idx, env,
                                                                                     atomic_exec_prompt,
                                                                                     past_action_checkpoint=[],
                                                                                     comm_state=False, verbose=verbose)
        rs.append(r)
        cnts.append(1)
        rate = sum(rs) / sum(cnts)
        outputs[f'env_{idx}'] = {'problem': task, 'commands': commands, 'trace': trace, 'plans': plans, 'reward': r,
                                 'runs': num_runs}
        ...
        outputs[f'env_{idx}'] = {'problem': task, 'commands': commands, 'trace': trace, 'plans': None, 'reward': 0,
                                 'runs': 0}
        print(f"idx={idx} exception: {e}")
