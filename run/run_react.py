...
def run_item(task, output_path="."):
    ...
    max_iter = MAX_ITER
    if type == 1:
        from agents.react_agent.prompt import EXAMPLE_MESSAGES_1
        ...
        from agents.react_agent.prompt import EXAMPLE_MESSAGES_2
        main_agent = ReactAgent(model_name=model_name, task=task,
                                record_path=f'{output_path}/react_agent_record.json',
                                example_message=EXAMPLE_MESSAGES_2,proxy=proxy,api_interval=api_interval)
    elif type == 3:
        from agents.react_agent.prompt import EXAMPLE_MESSAGES_3
        main_agent = ReactAgent(model_name=model_name, task=task,
                                record_path=f'{output_path}/react_agent_record.json',
...
def run(type, begin=0, end=99, step=1):
    ...
    output_dir = f'./output/travel/react/{model_name}/type{type}'
...
for type in [3]:
    if type == 1:
        run(type, begin=0, end=99, step=1)
    elif type == 2:
        run(type, begin=0, end=99, step=1)
    elif type == 3:
        run(type, begin=0, end=99, step=1)
